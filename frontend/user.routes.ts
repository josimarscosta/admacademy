...
router.get('/', roleMiddleware(['admin']), async (req: Request, res: Response) => {
  try {
    // Em um ambiente real, buscaríamos usuários no banco
    const users = [
      { id: '1', name: 'Usuário 1', email: 'usuario1@exemplo.com', role: 'student' },
      { id: '2', name: 'Usuário 2', email: 'usuario2@exemplo.com', role: 'teacher' },
      { id: '3', name: 'Usuário 3', email: 'usuario3@exemplo.com', role: 'student' }
    ];
...
// Obter usuário por ID (admin ou coordenador)
router.get('/:id', roleMiddleware(['admin', 'coordinator']), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
