...
def load_data_from_github(file_path):
    try:
        ...
        if file_path.endswith('.json'):
            ...
            return pd.read_excel(pd.io.common.BytesIO(response.content))
        else:
    ...
    with col2:
        ...
        if simulations:
            ...
            if completed_simulations:
                ...
                bars = ax.bar(df['title'], df['score'], color='#1E40AF')
                ax.set_xlabel('Simulado')
                ax.set_ylabel('Pontuação (%)')
                ax.set_ylim(0, 100)
                ax.set_title('Desempenho nos Simulados')
...
# Página de Simulados
    ...
    if not simulations:
        ...
        if completed_simulations:
            ...
            # Usando matplotlib em vez de plotly
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.plot(df['title'], df['score'], marker='o', linestyle='-', color='#1E40AF')
            ax.set_xlabel('Simulado')
            ax.set_ylabel('Pontuação (%)')
            ax.set_ylim(0, 100)
            ax.set_title('Evolução de Desempenho nos Simulados')
            plt.xticks(rotation=45, ha='right')
            plt.tight_layout()
...
# Página de Analytics
    ...
    if not simulations or not trails:
        ...
        with col1:
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            bars = ax.bar(areas.keys(), areas.values(), color='#1E40AF')
            ax.set_xlabel('Área')
            ax.set_ylabel('Pontuação (%)')
            ax.set_ylim(0, 100)
            ax.set_title('Desempenho por Área')
            plt.xticks(rotation=45, ha='right')
            plt.tight_layout()
        ...
        with col2:
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.plot(dates, scores, marker='o', linestyle='-', color='#1E40AF')
            ax.set_xlabel('Data')
            ax.set_ylabel('Pontuação (%)')
            ax.set_ylim(0, 100)
            ax.set_title('Evolução de Desempenho ao Longo do Tempo')
            plt.xticks(rotation=45, ha='right')
            plt.tight_layout()
